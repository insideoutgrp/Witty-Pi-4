...
# v4.42: ensure PATH includes sbin dirs. cron runs scripts with
...
if [ -z ${I2C_MC_ADDRESS+x} ]; then
  ...
  # v4.47: HTTPS, not HTTP. Cleartext http://google.com responses can be
...
has_internet()
{
  # v4.44: bump connect-timeout 3s -> 15s, add max-time. 3s is too short
  # for 3G/4G negotiation on field devices (matches checkInternet.sh's
  # PING_TIMEOUT=15). max-time bounds total wait.
...
get_rtc_timestamp()
{
  # v4.45: validate every register read. A single failed read previously
...
# v4.45: alarm reads are validated - a failed i2c_read no longer silently
...
# v4.45: alarm registers are written DAY FIRST (day, hour, min, sec).
# Newer firmware (Rev 13+) clears its ALARM*_TRIGGERED suppression flag on
# the FIRST byte of a rewrite, and with the old sec-first order the
# register block still encoded YESTERDAY's (in-window) alarm until the day
# byte landed - a firmware tick in that window hard-cut power mid-boot.
# Writing the day byte first makes every torn intermediate state
# future-dated for daily schedules. Harmless on older firmware (narrow 4s
# match window).
set_startup_time()
{
  date=$(dec2bcd $1)
...
net_to_system()
{
  ...
  if [[ "$net_ts" != "-1" ]]; then
    ...
    # v4.44: briefly disable NTP, slam the clock, then re-enable NTP.
    # Without the re-enable, a prior offline-boot's rtc_to_system
    ...
    # the manual `date -s`. set-ntp 1 is idempotent and harmless on
    # images without systemd-timesyncd.
    ...
    # v4.45: report failure so callers can SKIP system_to_rtc. Previously
...
rtc_to_system()
{
  ...
  local rtc_ts=$(get_rtc_timestamp)
  # v4.45: never write an unreadable RTC into the system clock
  if [ -z "$rtc_ts" ]; then
    log '  RTC is unreadable - keeping current system time.'
    return 1
  fi
  # v4.44: re-enable NTP after the manual set. Previously this function
  # called `timedatectl set-ntp 0` (so date -s wouldn't fight timesyncd)
  # but never re-enabled, leaving systemd-timesyncd permanently disabled.
  # Combined with the every-15-min HTTP-Date cron sync only giving 1s
  # resolution and no drift discipline, that left field devices unable
  # to recover from large clock errors: once NTP was off, drift
  # accumulated faster than the cron sync could catch — and if
  # has_internet ever failed on a tick (3G outage, 3s timeout, etc.)
  # the clock just kept walking.
  sudo timedatectl set-ntp 0 >/dev/null 2>&1
...
{
  # v4.45: rewritten to mirror the newer firmware's actual matching
  # semantics (day-of-month-relative timestamps with a backward 86400s
  # window on Rev 13+; a 4s window on older revisions). The old
  # "alarm-day < today means next month" heuristic classified
  # yesterday's stale alarm (the common overnight case) as
  # next-month-future and never caught it.
  #
  # Policy per kind:
  ...
  #   SHUTDOWN: LOG ONLY - never clear. On Rev 14 firmware the stale
  #     alarm is suppressed by ALARM2_TRIGGERED set at wake, and
  #     runScript.sh rewrites alarm2 right after the daemon anyway.
  #     Clearing here created a rewrite-from-zeros torn state that
  #     could race the firmware into a mid-boot power cut.
  ...
  if [ $overdue -ge -30 ] && [ $overdue -lt 86400 ]; then
    ...
    if [ "$kind" = "startup" ]; then
      ...
      log "NOTE: shutdown alarm '$raw' is stale (${overdue}s past); runScript will rewrite it."
    fi
...
i2c_read()
{
  ...
  if [[ $result =~ ^0x[0-9a-fA-F]{2}$ ]] ; then
    ...
    retry=$(( $retry + 1 ))
    if [ $retry -eq 4 ] ; then
      # v4.45: log to FILE only. log() echoes to stdout, and callers
...
{
  # v4.45: convert a raw alarm "DD HH:MM:SS" (UTC) to the epoch of its

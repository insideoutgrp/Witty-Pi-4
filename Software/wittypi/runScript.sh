...
# v4.45: serialise our alarm-register writes with the cron I2C jobs.
...
# v4.46: the lock is shared between root (daemon/cron) and the
...
# v4.45: remember why this boot happened - a Guaranteed Wake landing in
  ...
  if [ $begin == 0 ] ; then
    ...
    if [ $((cur_time >= end)) == '1' ] ; then
      ...
      do
        ...
        # v4.43: compare against the DST-corrected boundary, not raw epoch.
        # The loop walks check_time in raw seconds; setup_*_state applies
        # dst_correct() at the very end. During BST, raw boundaries land
        # 1h ahead of the intended local clock — so the comparison
        ...
        if [ $index == $count ] ; then
          ...
          if [ $script_duration == 0 ] ; then
            if [ $found_off == 0 ] ; then
              ...
              # v4.45: guard against zero cycle duration (malformed
      ...
      # v4.45: Guaranteed-Wake return-to-sleep. If the 24h backstop fired
      ...
      # the stay-up behaviour (an operator may be present). No-op on
      # older firmware, which never reports this wake reason.

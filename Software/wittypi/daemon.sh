...
# v4.46: create world-writable so the interactive wittyPi.sh menu (run as
...
if [ $has_mc == 1 ] ; then
  ...
  # Defensive override regardless of whatever the device was previously
  # configured with - users (or older deploys) may have set non-255
  # values that combine badly with IGNORE_LV_SHUTDOWN or LOW_VOLTAGE=255
  # to auto-wake during sleep.
